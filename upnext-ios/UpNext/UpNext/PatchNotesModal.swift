...
struct PatchNotesModal: View {
    ...
    static let notes: [PatchNoteVersion] = [
        PatchNoteVersion(
            version: "2026.05.22",
            date: "2026-05-22",
            headline: "네이티브 iOS 회복 — 폴라로이드·미니게임·전투 비주얼 전면 부활",
            entries: [
                PatchNoteEntry(icon: .image, title: "폴라로이드 기둥 회복",
                               description: "라이브 카메라 + 5종 프레임 + Kodak Gold 필터 + 스티커/서명/메모 + PNG 합성 공유까지 — 사진 인증 흐름이 네이티브로 돌아왔습니다."),
                PatchNoteEntry(icon: .gamepad, title: "미니게임 11종 작동",
                               description: "이전엔 자동 승리 처리되던 모든 반사 미니게임이 실제 입력으로 동작합니다. PairMatch·ReactionTap·TapBurst·PipeConnect 외 7종."),
                PatchNoteEntry(icon: .sword, title: "던전 전투 시각화",
                               description: "텍스트 로그에서 HeroSprite vs MonsterSprite 대치 + BossBanner + 14 부유 전투 숫자 + crit-shake + attack-flash + DungeonAtmosphere 까지."),
                PatchNoteEntry(icon: .sparkle, title: "전 화면 오로라 + 별",
                               description: "웹과 동일한 3-layer 오로라 앰비언트 + 1px 트윙클 별을 전 화면에 마운트."),
                PatchNoteEntry(icon: .gift, title: "카드팩 시네마틱",
                               description: "shake → flash → 3-halo expand → reveal 스태거 — 등급별 강도 차등."),
            ],
            isNew: true
        ),
        PatchNoteVersion(
            version: "2026.05.21",
            date: "2026-05-21",
            headline: "충실도 그리드 R0-R7 — 디자인 토큰·타이포·스프링 수학·SplashView 1:1 포팅",
            entries: [
                PatchNoteEntry(icon: .check, title: "디자인 토큰 1:1",
                               description: "웹 globals.css 의 모든 hex 값을 Color+Tokens.swift 에 정확히 옮김."),
                PatchNoteEntry(icon: .play, title: "Splash 모션",
                               description: "U↗Next 2.8s 시퀀스 — spring duration 0.7 bounce 0.3 을 SwiftUI response/dampingFraction 수학 변환."),
            ],
            isNew: false
        ),
    ]
