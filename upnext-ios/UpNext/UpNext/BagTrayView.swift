...
struct BagTrayView: View {
    ...
    /// Track E 합성 모드 — 재료로 고른 타일 / 재료가 될 수 없는 타일(흐리게).
    ...
    private func tile(_ item: Equipment) -> some View {
        ...
        return tileFace(item, selected: selected,
                        dimmed: lift?.item.id == item.id || dimmedIds.contains(item.id))
            .frame(width: Self.tile, height: Self.tile)
    ...
    private func tileFace(_ item: Equipment, selected: Bool, dimmed: Bool) -> some View {
        ...
        return ZStack {
            ...
            if enhance > 0 {
                // Track B 강화 칩 톤 (1..9 어둡게 / 10..14 골드 / 15+ 라임 글로우). 웹 BagTray 동일.
