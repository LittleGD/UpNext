 ...
 *  - large (보스) — pulsate
 *
 * Phase 6-E — 보스 24종은 `bossSprites.ts` 의 템플릿별 프레임을 쓴다 (templateId prop).
 *   kind 프레임 데이터는 `monsterFrames.ts` 로 옮겼다.
 */

import { memo, type CSSProperties } from "react";
import type { MonsterKind } from "@/types/uphero";
import { getMonsterFrames } from "./bossSprites";
...
interface MonsterSpriteProps {
  kind: MonsterKind;
  /**
   * Phase 6-E (Track E, 피드백 29) — 보스 템플릿 id. `BOSS_FRAMES` 에 있으면 보스 전용
   *   실루엣, 없으면(일반 몬스터·레거시 세이브) kind 프레임으로 폴백.
   */
  templateId?: string;
  size?: number;
  color?: string;
...
function MonsterSpriteInner({
  ...
  const [frame1, frame2] = getMonsterFrames(kind, templateId);
