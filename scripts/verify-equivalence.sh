...
# Game.swift(Language)가 AppConfig(Shared, Foundation-only)를 참조하므로(19-i18n-mixed)
# 모든 suite 컴파일에 함께 포함한다 — 없으면 'cannot find AppConfig' 로 전 suite 실패.
...
# ⚠️ sync suite — UserDoc.retention(RetentionState) 추가 이후 컴파일 불가.
#   Retention.swift → GrowthModels.swift(PhotoMeta) → StickerLayer.swift(Sticker, UIKit)로
#   의존이 UI 레이어까지 이어져 CLI swiftc 로 못 묶는다. Sticker 모델을 Models/ 로
#   분리해야 복구 가능 — 별도 작업으로 유보.
